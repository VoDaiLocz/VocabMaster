...
export const useDeckStore = create<DeckStore>((set, get) => ({
  ...
  createWord: async (word: NewWord) => {
    try {
      ...
      if (result.lastId > 0) {
        await Promise.all([
          window.electronAPI.dbRun('INSERT INTO progress (word_id) VALUES (?)', [result.lastId]),
          window.electronAPI.dbRun('UPDATE decks SET word_count = word_count + 1 WHERE id = ?', [
            word.deck_id,
          ]),
        ])
      }

      await Promise.all([get().fetchWords(word.deck_id), get().fetchDecks()])
    } catch (e) {
   ...
   */
  importWordsWithDeduplication: async (
    deckId: number,
    words: ImportWord[],
    onProgress?: ImportProgressCallback,
  ) => {
