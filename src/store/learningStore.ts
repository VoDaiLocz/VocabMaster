...
export const useLearningStore = create<LearningStore>((set, get) => ({
  ...
  fetchTodayWords: async (deckId?: number) => {
    ...
    try {
      ...
      const words = deckId ? await fetchDeckWords(deckId) : await fetchReviewWords(today)
  ...
  fetchUserStats: async () => {
    try {
      const [streakResult, xpResult] = await Promise.all([
        window.electronAPI.dbGet<{ value: string }>('SELECT value FROM settings WHERE key = ?', [
  ...
  updateStreak: async () => {
    ...
    try {
      ...
      await Promise.all([
        window.electronAPI.dbRun('UPDATE settings SET value = ? WHERE key = ?', [
          newStreak.toString(),
          DB.SETTINGS_KEYS.STREAK,
        ]),
        window.electronAPI.dbRun('UPDATE stats SET streak_maintained = 1 WHERE date = ?', [today]),
      ])
...
async function ensureProgressRecords(): Promise<void> {
  ...
  for (const w of wordsWithoutProgress) {
    await window.electronAPI.dbRun("INSERT INTO progress (word_id, status) VALUES (?, 'new')", [
      w.id,
    ])
  }
...
async function updateWordProgress(
  ...
  await window.electronAPI.dbRun('INSERT OR IGNORE INTO progress (word_id, status) VALUES (?, ?)', [
    wordId,
    'new',
  ])
...
async function updateDailyStats(today: string, quality: Quality, xp: number): Promise<void> {
  await window.electronAPI.dbRun(
    `
    UPDATE stats SET 
...
async function checkAchievements(responseTime: number, quality: Quality): Promise<void> {
