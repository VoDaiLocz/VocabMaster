 ...
 */
function determineStatus(repetitions: number, leitnerBox: number, interval: number): WordStatus {
 ...
 */
export function getXPForQuality(quality: Quality, streak = 0, isQuiz = false): number {
...
export function calculateLevel(totalXP: number): LevelInfo {
  ...
  const progress =
    nextLevelXP > prevLevelXP ? ((totalXP - prevLevelXP) / (nextLevelXP - prevLevelXP)) * 100 : 100
