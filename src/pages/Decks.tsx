...
const DeckGrid = memo(function DeckGrid({ decks, onStudy, onDelete }: DeckGridProps) {
  return (
    <div className='grid grid-cols-3 gap-4'>
      {decks.map((deck) => (
        <DeckCard key={deck.id} deck={deck} onStudy={onStudy} onDelete={onDelete} />
      ))}
    </div>
  )
})
    ...
    <Modal isOpen={isOpen} onClose={onClose} title='Tạo bộ từ mới'>
      <div className='space-y-4'>
            ...
            {DECK_COLORS.map((color) => (
              <button
                ...
                className={`w-8 h-8 rounded-full transition-transform ${
                  form.color === color ? 'ring-2 ring-offset-2 ring-primary-500 scale-110' : ''
                }`}
        ...
        <div className='flex gap-3 pt-4'>
          ...
          <Button className='flex-1' onClick={onCreate} disabled={!form.name.trim()}>
            Tạo
          </Button>
        </div>
      </div>
