...
export function calculateToeicScore(
  ...
  const normListeningRaw =
    listeningTotal > 0 ? Math.round((listeningRaw / listeningTotal) * 100) : 0
  ...
  if (partBreakdown[2].percentage < 70) {
    recommendations.push(
      'Part 2 (Hỏi & Đáp) cần rèn luyện phản xạ né bẫy trả lời gián tiếp và bẫy lặp từ/đồng âm.',
    )
  }
  if (partBreakdown[3].percentage < 70 || partBreakdown[4].percentage < 70) {
    recommendations.push(
      'Part 3 & 4 cần đọc trước 3 câu hỏi trước khi audio phát để bắt trúng từ khóa trong bài nói.',
    )
  }
  if (partBreakdown[5].percentage < 75) {
    recommendations.push(
      'Part 5 cần củng cố 17 chuyên đề ngữ pháp, đặc biệt là chia thì động từ và từ loại danh-tính-động-trạng.',
    )
  }
  if (partBreakdown[7].percentage < 70) {
    recommendations.push(
      'Part 7 cần tối ưu tốc độ đọc lướt (Skimming & Scanning) cho đoạn kép và đoạn ba để tránh thiếu giờ.',
    )
  }
  if (recommendations.length === 0) {
    recommendations.push(
      'Phong độ xuất sắc! Hãy duy trì luyện giải các đề ETS 2024 mới nhất để giữ nhịp phản xạ.',
    )
  }
