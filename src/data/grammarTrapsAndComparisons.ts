...
export interface GrammarExamTrap {
  ...
  trapTitle: string
  trapType:
    'Word Choice' | 'Tense Confusion' | 'Structure Trap' | 'Agreement Trap' | 'Passive Distortion'
...
export const COMPARISON_MATRICES: ComparisonMatrixItem[] = [
  {
    ...
    conceptA: {
      ...
      usage:
        'Diễn tả hành động xảy ra trong quá khứ nhưng KHÔNG nêu rõ thời gian cụ thể, hoặc hành động bắt đầu trong quá khứ và vẫn đang tiếp diễn/để lại kết quả ở hiện tại.',
      exampleEn: 'Mr. Tanaka has managed the Tokyo branch for over five years.',
      exampleVi:
        'Ông Tanaka đã và đang quản lý chi nhánh Tokyo được hơn năm năm rồi (hiện tại vẫn quản lý).',
    },
    conceptB: {
      name: 'Quá Khứ Đơn (V2/ed / did not + V_inf)',
      formula: 'S + V2/ed',
      usage:
        'Diễn tả hành động đã chấm dứt hoàn toàn tại một mốc thời gian xác định trong quá khứ (yesterday, last month, ago, in 2021).',
      exampleEn: 'Mr. Tanaka managed the Tokyo branch from 2015 to 2020.',
      exampleVi:
        'Ông Tanaka đã quản lý chi nhánh Tokyo từ năm 2015 đến 2020 (nay không còn quản lý nữa).',
    },
    keyDifferentiator:
      'Dấu hiệu thời gian quyết định: Nếu câu có "since / for / recently / yet / already" -> Hiện tại hoàn thành; nếu có mốc quá khứ dứt khoát "yesterday / last / ago / in [năm quá khứ]" -> BẮT BUỘC dùng Quá khứ đơn.',
    examTrapWarning:
      'Bẫy ETS thường đưa ra từ "recently / lately" (dùng HTHT) nằm xen kẽ với "last week" để gây nhầm lẫn; hoặc đưa "since + mốc quá khứ" khiến thí sinh nhầm tưởng chọn V2 thay vì have/has V3 cho mệnh đề chính.',
  },
  {
    id: 'cmp-02',
    ...
    conceptA: {
      ...
      usage:
        'Theo sau bắt buộc phải là một MỆNH ĐỀ HOÀN CHỈNH có đầy đủ Chủ ngữ và Động từ chia thì.',
    ...
    conceptB: {
      ...
      usage:
        'Theo sau bắt buộc là một CỤM DANH TỪ hoặc DANH ĐỘNG TỪ (V-ing), TUYỆT ĐỐI KHÔNG đi với một mệnh đề.',
    ...
    keyDifferentiator:
      'Kiểm tra phía sau chỗ trống: Có động từ chia thì (conjugated verb) hay chỉ là danh từ/cụm danh từ. Nếu có động từ chia thì -> Chọn liên từ (Although). Nếu chỉ có cụm từ -> Chọn giới từ (Despite).',
    examTrapWarning:
      'ETS rất hay gài cụm danh từ dài có mệnh đề quan hệ rút gọn phía sau (ví dụ: Despite the strict policies implemented last month,...), thí sinh thấy từ "implemented" tưởng là động từ chia thì nên chọn Although là rơi vào bẫy.',
  },
  {
    id: 'cmp-03',
    ...
    keyDifferentiator:
      'Cấu trúc phía sau: Clause (S + V) -> Because/Since/As; Noun Phrase -> Because of / Due to / Owing to.',
    examTrapWarning:
      'Cực kỳ lưu ý cụm "Due to the fact that + S + V" tương đương với "Because + S + V". Nếu chỉ có "Due to" thì chỉ đi với danh từ.',
  },
  {
    id: 'cmp-04',
    ...
    conceptA: {
      ...
      usage:
        'Rút gọn mệnh đề quan hệ chủ động hoặc mệnh đề trạng ngữ đồng chủ ngữ khi chủ ngữ là tác nhân gây ra hành động.',
      exampleEn: 'The committee reviewing the proposals will announce the winner tomorrow.',
      exampleVi:
        'Hội đồng (người đang xem xét các đề xuất) sẽ công bố người chiến thắng vào ngày mai.',
    },
    conceptB: {
      name: 'Quá Khứ Phân Từ - Dạng Bị Động (V-ed / V3)',
      formula: 'V-ed/V3 (+ by Object) (khi chủ ngữ bị/được tác động)',
      usage:
        'Rút gọn mệnh đề quan hệ bị động hoặc mệnh đề trạng ngữ khi chủ ngữ chịu tác động từ bên ngoài.',
      exampleEn: 'The revised proposal submitted by Mr. Henderson was unanimously approved.',
      exampleVi:
        'Bản đề xuất đã được chỉnh sửa (được nộp bởi ông Henderson) đã được phê duyệt nhất trí.',
    },
    keyDifferentiator:
      'Xem danh từ được bổ nghĩa là chủ thể gây ra hành động (phía sau thường có tân ngữ O -> chọn V-ing) hay danh từ đó bị tác động (phía sau thường có giới từ hoặc không có tân ngữ -> chọn V-ed/V3).',
    examTrapWarning:
      'Các động từ chỉ cảm xúc (interest, bore, excite, satisfy): V-ing mô tả tính chất sự vật/sự việc; V-ed mô tả cảm xúc/tâm trạng của con người.',
  },
  {
    id: 'cmp-05',
    ...
    conceptA: {
      ...
      usage:
        'Bắt buộc động từ mệnh đề sau "that" phải ở dạng NGUYÊN THỂ KHÔNG "TO", bất kể chủ ngữ là số ít (he/she/it) hay thời gian ở quá khứ.',
      exampleEn: 'The board requested that every manager submit the financial audit by Friday.',
      exampleVi:
        'Hội đồng quản trị yêu cầu mọi trưởng phòng phải nộp báo cáo kiểm toán trước thứ Sáu (submit giữ nguyên thể dù manager là số ít).',
    },
    conceptB: {
    ...
    keyDifferentiator:
      'Nếu mệnh đề chính chứa các động từ/tính từ chỉ sự yêu cầu/khuyên bảo (mandate, require, recommend, ask, suggest, essential, imperative, crucial) + that -> Động từ mệnh đề sau LUÔN Ở DẠNG NGUYÊN MẪU KHÔNG CHIA.',
    examTrapWarning:
      'ETS luôn đưa ra phương án chia thì số ít (submits) hoặc quá khứ (submitted) để bẫy người làm theo phản xạ thấy "manager" chọn "submits".',
  },
  {
    id: 'cmp-06',
    ...
    conceptB: {
      ...
      formula:
        'Others (những người/vật khác không xác định) / The others (những người/vật còn lại đã xác định)',
      usage:
        'Đóng vai trò là Chủ ngữ hoặc Tân ngữ đứng một mình, KHÔNG BAO GIỜ đi kèm danh từ theo sau.',
      exampleEn: 'Some participants liked the seminar, while others found it repetitive.',
      exampleVi:
        'Một số người tham dự thích buổi hội thảo, trong khi những người khác thấy nó lặp lại.',
    },
    keyDifferentiator:
      'Others có "s" thì ĐỨNG MỘT MÌNH không có danh từ theo sau. Other không có "s" thì PHẢI CÓ DANH TỪ SỐ NHIỀU đi kèm.',
    examTrapWarning:
      'Bẫy kinh điển: Đề bài cho chỗ trống trước danh từ số nhiều "____ candidates", phương án đưa ra cả "Others" và "Other". Người học vội vàng thấy "candidates" số nhiều liền chọn "Others" là sai hoàn toàn (đáp án đúng phải là "Other candidates").',
  },
]
...
export const GRAMMAR_EXAM_TRAPS: GrammarExamTrap[] = [
  ...
  },
  {
    ...
    deepAnalysis:
      'Trong tiếng Anh, mệnh đề phụ chỉ thời gian bắt đầu bằng When, As soon as, Once, Until, Before, After TUYỆT ĐỐI KHÔNG dùng thì tương lai (will/shall). Phải dùng Hiện Tại Đơn (hoặc Hiện Tại Hoàn Thành) để thay thế.',
    etsTestFrequency: 'Rất cao (90%)',
  },
  {
    id: 'trap-03',
    ...
    incorrectExample:
      '❌ The installation of the new manufacturing machines were completed on time.',
    correctExample: '✅ The installation of the new manufacturing machines was completed on time.',
    deepAnalysis:
      'Chủ ngữ thật của câu là danh từ đứng trước giới từ: "The installation" (danh từ số ít), chứ không phải "machines" (danh từ số nhiều nằm trong cụm giới từ "of the new manufacturing machines"). Do đó động từ phải chia số ít: "was completed".',
    etsTestFrequency: 'Rất cao (90%)',
  },
  {
    id: 'trap-04',
  ...
  },
  {
    ...
    deepAnalysis:
      'Các nội động từ (Intransitive Verbs) như: occur, happen, appear, disappear, remain, exist, rise, fall, arrive KHÔNG BAO GIỜ chia ở thể bị động vì chúng không nhận tân ngữ trực tiếp.',
  ...
  },
  {
    ...
    deepAnalysis:
      'Vị trí nằm giữa trợ động từ (have/has/be) và động từ chính (V3/ed) LUÔN LUÔN là một TRẠNG TỪ (Adverb - đuôi ly) để bổ nghĩa cho động từ chính đó: S + have/be + [ADV] + V3/ed.',
    etsTestFrequency: 'Rất cao (90%)',
  },
  {
    id: 'trap-07',
    ...
    trapTitle:
      'Bẫy Rút Gọn Mệnh Đề Quan Hệ Với Danh Từ Có Mạo Từ "The Only / The First / The Best"',
    ...
    deepAnalysis:
      'Khi danh từ được bổ nghĩa đứng trước có các từ hạn định chỉ thứ tự hoặc độc nhất như: the first, the second, the last, the only, hoặc so sánh nhất (the best, the most), khi rút gọn mệnh đề quan hệ ta BẮT BUỘC dùng TO-INFINITIVE (to + V_inf), KHÔNG dùng V-ing.',
    etsTestFrequency: 'Cao (75%)',
  },
  {
    id: 'trap-08',
