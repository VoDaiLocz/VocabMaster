...
export class SmartBookmarkValidator {
  static validateBookmark(bookmark: Partial<BookmarkRecord>): BookmarkValidationResult {
    const errors: string[] = [];
    if (!bookmark.id || typeof bookmark.id !== "string") errors.push("Bookmark id is required and must be a string");
    if (!bookmark.storyId || typeof bookmark.storyId !== "string") errors.push("storyId is required");
    if (!bookmark.chapterId || typeof bookmark.chapterId !== "string") errors.push("chapterId is required");
    if (typeof bookmark.scrollPositionPercent !== "number" || bookmark.scrollPositionPercent < 0 || bookmark.scrollPositionPercent > 100) {
