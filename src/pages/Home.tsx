  ...
  return (
    <div className='p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8'>
      ...
      {todayStats && (todayStats.quiz_score > 0 || todayStats.typing_score > 0) && (
        <TodayScoresCard quizScore={todayStats.quiz_score} typingScore={todayStats.typing_score} />
      )}
    </div>
  )
}
...
const HeaderSection = memo(function HeaderSection({ streak, level }: HeaderSectionProps) {
  return (
    <div className='flex items-center justify-between'>
      <div>
        <h1 className='text-2xl font-bold text-gray-800 dark:text-white'>{getGreeting()} 👋</h1>
      ...
      <div className='flex items-center gap-4'>
        ...
        >
          ...
          <span className='font-semibold text-purple-600 dark:text-purple-400'>Lv.{level}</span>
        </Link>
        <div className='flex items-center gap-2 bg-orange-100 dark:bg-orange-900/30 px-4 py-2 rounded-full'>
          <Flame className='text-orange-500' size={20} />
          <span className='font-semibold text-orange-600 dark:text-orange-400'>{streak} ngày</span>
        </div>
      </div>
    </div>
  )
