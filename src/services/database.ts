...
export async function getWordsForReview(limit?: number): Promise<(Word & { progress: Progress })[]> {
    const today = new Date().toISOString().split('T')[0]
    const tx = getDB().transaction(['words', 'progress'], 'readonly')
    const wordStore = tx.objectStore('words')
    const progressStore = tx.objectStore('progress')

    // Optimized: Fetch all data at once instead of O(n) individual lookups
    const [allWords, allProgress] = await Promise.all([
        promisifyRequest(wordStore.getAll()),
        promisifyRequest(progressStore.getAll())
    ])
...
// Counts - Optimized using indexed queries
export async function getProgressCounts(): Promise<{ new: number; review: number; mastered: number }> {
    const tx = getDB().transaction('progress', 'readonly')
    const store = tx.objectStore('progress')

    // Use IDBKeyRange to efficiently count by status
    const newCount = await countByStatus(store, 'new')
    const learningCount = await countByStatus(store, 'learning')
    const reviewCount = await countByStatus(store, 'review')
    const masteredCount = await countByStatus(store, 'mastered')

    return {
        new: newCount,
        review: learningCount + reviewCount,
        mastered: masteredCount
    }
}

async function countByStatus(store: IDBObjectStore, status: string): Promise<number> {
    return new Promise((resolve, reject) => {
