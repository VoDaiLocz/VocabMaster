...
import { BookOpen, Search, Clock, ChevronRight, Star, Layers } from 'lucide-react'
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <h1 className='text-2xl sm:text-3xl font-extrabold font-display'>
                Đọc Tiểu Thuyết Song Ngữ Anh - Việt
              </h1>
            </div>
          </div>
          <p className='text-sm sm:text-base text-emerald-100 max-w-2xl'>
            Đắm chìm vào thế giới Tiên Hiệp Tu Chân mưu lược đỉnh cao. 2 chế độ đọc: Chạm để mở bản
            dịch từng đoạn nhỏ & Song ngữ Anh - Việt đan xen.
          </p>
          ...
          <div className='pt-2 max-w-xl'>
            <div className='relative'>
              ...
              <input
                ...
                placeholder='Tìm kiếm tiểu thuyết theo tên, từ khóa...'
      ...
      <div className='max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 space-y-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {filteredStories.map((story) => {
            const totalWords = story.chapters.reduce((sum, c) => sum + c.wordCount, 0)
            const totalMins = story.chapters.reduce((sum, c) => sum + c.estimatedMinutes, 0)
            ...
            return (
              <Link
                ...
                to={'/story/' + story.id}
                className='group rounded-3xl bg-white dark:bg-dark-card border border-gray-100 dark:border-gray-800 hover:border-emerald-300 dark:hover:border-emerald-700/60 shadow-sm hover:shadow-xl transition-all cursor-pointer flex flex-col overflow-hidden no-underline text-inherit'
              >
                {/* Cover Image */}
                <div className='relative h-48 overflow-hidden bg-gray-900'>
                  ...
                  <div className='absolute top-3 right-3 px-2.5 py-1 rounded-xl bg-emerald-600/90 backdrop-blur-md text-[11px] font-bold text-white shadow-sm flex items-center gap-1'>
                    <Layers size={12} /> {story.chapters.length} Chương
                  </div>
                  ...
                  {story.tags && (
                    <div className='flex flex-wrap gap-1'>
                      {story.tags.map((t: string) => (
                        <span
                          key={t}
                          className='px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 text-[10px] font-semibold border border-emerald-200/50 dark:border-emerald-800/40'
                        >
                          #{t}
                        </span>
                      ))}
