...
export class ImageProcessor {
  ...
  private parseVocabulary(text: string): ImportWord[] {
    ...
    for (const line of lines) {
      ...
      const cleanedLine = this.preProcessLine(line)
  ...
  private intelligentParse(line: string): ImportWord | null {
    ...
    const beMatch = line.match(
      /^(be\s+[a-z]{3,}(?:\s+(?:sb|sth|over|into|out|in|on|up|away|after|before))?)\s+(.*)$/i,
    )
    if (beMatch) {
      const term = beMatch[1]
      const def = beMatch[2]
  ...
  private createImportWord(term: string, definition: string): ImportWord | null {
    ...
    const finalDefinition = cleanDefinition.replace(/\[([^\]]+)\]/g, '').trim()
