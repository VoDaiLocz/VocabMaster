    ...
    <div className='h-full flex flex-col p-8'>
      ...
      <div className='flex-1 flex flex-col items-center justify-center'>
        <div className='w-full max-w-xl'>
          ...
          {showResult && result && (
            <ResultFeedback result={result} correctAnswer={currentChallenge.word.term} />
          )}
...
const CompleteState = memo(function CompleteState({ score, onRestart }: CompleteStateProps) {
  ...
  const colorClass =
    percentage >= 80 ? 'bg-green-500' : percentage >= 50 ? 'bg-primary-500' : 'bg-yellow-500'
...
const ChallengeCard = memo(function ChallengeCard({ challenge }: ChallengeCardProps) {
  ...
  return (
    <div className='bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-lg mb-6 text-center'>
      ...
      <p className='text-2xl font-medium text-gray-800 dark:text-white mb-4'>{challenge.hint}</p>
  ...
  return (
    <div className={`mt-4 p-4 rounded-xl text-center ${bgClass}`}>
      ...
      {result.similarity > 0 && result.similarity < 100 && (
        <p className='text-sm text-gray-500 mt-1'>Độ chính xác: {result.similarity}%</p>
      )}
    </div>
  )
})
